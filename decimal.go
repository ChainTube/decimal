...
func newFromFloat(val float64, bits uint64, flt *floatInfo) Decimal {
	...
	if d.nd < 19 {
		...
		return Decimal{value: big.NewInt(tmp), exp: int32(d.dp) - int32(d.nd), decimalsShift: shiftStorageDecimals}
	}
	dValue := new(big.Int)
	dValue, ok := dValue.SetString(string(d.d[:d.nd]), 10)
	if ok {
		return Decimal{value: dValue, exp: int32(d.dp) - int32(d.nd), decimalsShift: shiftStorageDecimals}
	}
...
func (d Decimal) QuoRem(d2 Decimal, precision int32) (Decimal, Decimal) {
	...
	var q, r big.Int
	q.QuoRem(&aa, &bb, &r)
	dq := Decimal{value: &q, exp: scale, decimalsShift: d.decimalsShift}
	dr := Decimal{value: &r, exp: scalerest, decimalsShift: d.decimalsShift}
	return dq, dr
}
...
func (d Decimal) DivRound(d2 Decimal, precision int32) Decimal {
	...
	r2 := Decimal{value: &rv2, exp: r.exp + precision, decimalsShift: d.decimalsShift}
...
func (d Decimal) Pow(d2 Decimal) Decimal {
	...
	if baseSign == 0 {
		...
		if expSign == 1 {
			return Decimal{value: zeroInt, exp: 0, decimalsShift: shiftStorageDecimals}
		}
		if expSign == -1 {
			return Decimal{}
		}
	...
	if expSign == 0 {
		return Decimal{value: oneInt, exp: 0, decimalsShift: shiftStorageDecimals}
	}

	// TODO: optimize extraction of fractional part
	one := Decimal{value: oneInt, exp: 0, decimalsShift: shiftStorageDecimals}
...
func (d Decimal) PowWithPrecision(d2 Decimal, precision int32) (Decimal, error) {
	...
	if baseSign == 0 {
		...
		if expSign == 1 {
			return Decimal{value: zeroInt, exp: 0, decimalsShift: shiftStorageDecimals}, nil
		}
		if expSign == -1 {
	...
	if expSign == 0 {
		return Decimal{value: oneInt, exp: 0, decimalsShift: shiftStorageDecimals}, nil
	}
...
func (d Decimal) ExpHullAbrham(overallPrecision uint32) (Decimal, error) {
	...
	if d.Abs().Cmp(overflowThreshold2) <= 0 {
		return Decimal{value: oneInt, exp: d.exp, decimalsShift: d.decimalsShift}, nil
	}
	...
	r := Decimal{value: new(big.Int).Set(d.getValue()), exp: d.exp - t, decimalsShift: d.decimalsShift} // reduced argument
...
func (d Decimal) ExpTaylor(precision int32) (Decimal, error) {
	...
	if d.IsZero() {
		return Decimal{value: oneInt, exp: 0, decimalsShift: shiftStorageDecimals}.Round(precision), nil
	}
...
func (d Decimal) Ln(precision int32) (Decimal, error) {
	...
	comp1 = z.Sub(Decimal{value: oneInt, exp: 0, decimalsShift: shiftStorageDecimals})
	comp3 = Decimal{value: oneInt, exp: -1, decimalsShift: shiftStorageDecimals}
	...
	epsilon := Decimal{value: oneInt, exp: -calcPrecision, decimalsShift: shiftStorageDecimals}
	...
	if usePowerSeries {
		...
		// z + 2
		comp2 = comp1.Add(Decimal{value: twoInt, exp: 0, decimalsShift: shiftStorageDecimals})
		// z / (z + 2)
...
func (d Decimal) Floor() Decimal {
	...
	return Decimal{value: z, exp: 0, decimalsShift: d.decimalsShift}
}
...
func (d *Decimal) UnmarshalJSON(decimalBytes []byte) error {
	// fork change: tolerate the empty JSON string `""` and the `null` literal
	// — emitted by WooCommerce and other lax APIs for unset numeric fields.
	// Note: unquoteIfQuoted requires len > 2 to strip, so the 2-byte `""` will
	// not be stripped down to "" — we check the raw bytes here.
	switch string(decimalBytes) {
	case "null", `""`:
		return nil
	}

	str := unquoteIfQuoted(string(decimalBytes))
	if str == "" {
		return nil
	}
...
func (d *Decimal) Scan(value interface{}) error {
	...
	case int64:
		...
		// fork change: this is the default type we get from DB for our shifted decimals
		if v == 0 {
			*d = New(v, 0)
		} else {
			*d = NewFromFloat32(float32(number.ShiftIntToFloat(v, shiftStorageDecimals)))
		}
		return nil
	...
	case string:
		s := unquoteIfQuoted(v)
		if s == "" || s == "null" {
			// fork change: tolerate empty / "null" text from DBs that store
			// decimals as text columns. Treat as zero rather than erroring.
			*d = Zero
			return nil
		}
		var err error
...
// fork change: serializes the decimal as a shifted int64 so it can be stored in
// BIGINT columns. Use ToShiftedInt to get the raw shifted integer.
...
// NewFromShiftedInt returns a new Decimal from a shifted integer using the global
// shiftStorageDecimals setting. Useful when reading numbers from database with raw queries.
func NewFromShiftedInt(nr int64) (Decimal, error) {
	dec := NewFromFloat32(float32(number.ShiftIntToFloat(nr, shiftStorageDecimals)))
	return dec, nil
}

// NewFromShiftedIntWithShiftCount returns a new Decimal from a shifted integer
// with the given number of decimals to shift. Useful when reading numbers from
// database with raw queries when the shift differs from the global setting.
...
func (d *Decimal) UnmarshalText(text []byte) error {
	str := string(text)
	if str == "" {
		// fork change: tolerate the empty text payload — emitted by XML and
		// other text wire formats for unset numeric fields. Treat as zero
		// (mirrors UnmarshalJSON and the existing NullDecimal.UnmarshalText guard).
		return nil
	}
